...
g1 <- ggplot(data=g1_data, aes(x = g1_data$yr, y = pct, group=povertycategory, col=povertycategory)) +
  ...
  scale_x_continuous(name="School year", breaks=seq(2002, 2018, 2))+
    theme_hc()+
...
```{r}
enroll_special_3 %>%  filter(location_new=='CORE CITIES', povertycategory=='Very High' | povertycategory=='High') %>% group_by(yr) %>% summarise(kids=sum(K12Enr), count=n())%>%  kable("html") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"), full_width = F, position="left")
```
